...
async function searchForMenuUrls(query: string, timeoutMs: number): Promise<any> {
  ...
  if (!res.ok) {
    ...
    console.error('[Meet My Menu AI] find-menu search error:', res.status, text.slice(0, 500));
...
async function pickBestSource(urls: string[], remaining: () => number): Promise<MenuSource | null> {
  ...
  for (const url of urls) {
    ...
    try {
      ...
      console.error('[Meet My Menu AI] find-menu candidate fetch failed:', url, e);
    }
  }
  return best;
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.error('[Meet My Menu AI] find-menu error:', e);
