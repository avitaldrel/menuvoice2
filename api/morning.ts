...
//   1. Did anyone use Meet My Menu AI in the window?  (yes / no)
...
function emailSubject(d: { anyoneUsed: boolean; newUsers: unknown[]; returningUsers: unknown[]; website?: { visits: number }; cartesia: { allExhausted: boolean } }): string {
  ...
  return d.cartesia.allExhausted
    ...
    : d.anyoneUsed
    ? `[Meet My Menu AI] Morning report ${date} — ${d.newUsers.length} new, ${d.returningUsers.length} returning, ${d.website?.visits ?? 0} site visits`
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const verdict = d.anyoneUsed
      ? `<p class="verdict yes">Yes — Meet My Menu AI was used by <strong>${esc(t.users)}</strong> ${Number(t.users) === 1 ? 'person' : 'people'} (${esc(t.sessions)} session${Number(t.sessions) === 1 ? '' : 's'}).</p>`
      : `<p class="verdict no">No one used Meet My Menu AI in this window.</p>`;
...
<title>Meet My Menu AI morning report (${esc(d.windowLabel)})</title>
<style>
...
<body>
<h1>Meet My Menu AI morning report</h1>
