...
<title>Meet My Menu AI live dashboard</title>
<style>
...
<body>
<div class="topbar">
  <h1>Meet My Menu AI live dashboard</h1>
...
function renderCartesia(d){
  ...
  const creditNote=hasLiveCredits
    ? '<p class="small credit-note">Accounts marked Cartesia usage API use live usage totals and their configured allowance. Accounts marked Meet My Menu AI tracked estimate start at 20,000 and subtract successful Meet My Menu AI TTS requests at approximately one credit per character.</p>'
    : hasTrackedCredits
      ? '<p class="small credit-note"><strong>Each free key starts at 20,000 credits.</strong> Credits left are a Meet My Menu AI tracked estimate based on successful TTS requests at approximately one credit per character. Tracking begins with this deployment and cannot see usage outside Meet My Menu AI; Cartesia may vary slightly.</p>'
  ...
  const rows=(c.keys||[]).map(k=>'<tr><td>'+esc(k.email||'Email not stored')+'</td><td>'+esc(k.label)+'</td><td class="'+(k.status==='exhausted'?'bad':k.status==='active'?'good':'')+'">'+esc(k.status)+'</td><td class="r">'+creditCell(k,'remaining')+'</td><td class="r">'+creditCell(k,'used')+'</td><td class="r">'+(k.credits&&k.credits.limit!=null?num(k.credits.limit):'')+'</td><td>'+esc(k.credits&&k.credits.state==='live'?'Cartesia usage API':k.credits&&k.credits.state==='tracked'?'Meet My Menu AI tracked estimate':'Unavailable')+'</td><td>'+esc(k.credits&&k.credits.periodStart?fmtTs(k.credits.periodStart):'')+'</td><td>'+esc(fmtTs(k.lastSuccessAt))+'</td><td>'+esc(fmtTs(k.exhaustedAt))+'</td><td>'+esc(fmtTs(k.availableAt))+'</td></tr>').join('');
