...
export function PauseProvider({ children }: { children: React.ReactNode }) {
  ...
  const pause = useCallback((message = 'Voice paused. Meet My Menu stopped speaking and the microphone is off. Your conversation is saved.') => {
    stopSpeaking();
  ...
  const resume = useCallback((message = 'Voice resumed. The microphone is on and Meet My Menu can speak again.') => {
    setPaused(false);
    setStatus(message);
  }, []);
