...
// step to step with the rotor. A "Read tutorial aloud" button narrates the whole
// thing for users who rely entirely on audio.
...
const STEPS: Step[] = [
  ...
  },
  {
    ...
    body: 'Add your allergies in Settings. Any dish that may contain one of them shows a red Alert at the top and is read first, so you always hear the warning before the dish. Dishes are never hidden, and warnings only appear for allergens you listed. Always confirm with the restaurant, since alerts are based on the dish description.',
  },
  {
    title: 'Pause anytime',
...
export default function TutorialScreen({ navigate, goBack }: ScreenProps) {
  ...
  const fullText =
    'How MenuVoice works. ' +
    STEPS.map((s, i) => `Step ${i + 1}. ${s.title}. ${s.body}`).join(' ');
  ...
  return (
    <Screen>
      ...
      <Body>Six quick steps. You can read them, or have them read to you.</Body>

      <PrimaryButton
        label={reading ? 'Stop reading' : 'Read tutorial aloud'}
        hint={reading ? 'Stop the narration' : 'Have MenuVoice read this tutorial to you'}
        onClick={toggleRead}
      />
