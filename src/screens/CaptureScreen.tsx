...
export default function CaptureScreen({
    ...
    if (videoRef.current) {
      autoRef.current.start(videoRef.current, {
        ...
        onAutoZoom: (direction) => {
          ...
          // Reserve the value synchronously so scanner ticks cannot queue the
          // same zoom step while native applyConstraints is still pending.
          ...
          if (range.native) {
            void setCameraZoom(streamRef.current, next).then((applied) => {
              if (!applied) {
                ...
                // Stop retrying an unsupported direction. The scanner will
                // fall back to spoken guidance on its next pass.
  ...
  const changeZoom = async (direction: 1 | -1) => {
    ...
    if (range.native && !native) {
      const msg = 'This camera could not change zoom.';
      setStatus(msg);
      return;
    }
